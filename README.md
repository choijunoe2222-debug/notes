...
- 구분자가 사라진 독립 LaTeX 문단은 괄호·환경의 짝과 수식 구조를 검사한 뒤 복구합니다. 따라서 수식 중간의 `=`가 큰 제목으로 바뀌지 않습니다. 일반 Setext 제목과 코드 블록은 그대로 지원합니다.
- HTML에 수식 annotation이 없고 일반 텍스트에 LaTeX가 남아 있으면, HTML 변환으로 역슬래시를 다시 이스케이프하지 않고 텍스트를 보존합니다. 일반 문장과 섞인 모호한 수식이나 밀린 `**` 위치는 추측하여 고치지 않습니다.
...
iPad에서 보고된 boxed/multiline/matrix/aligned 입력은 같은 테스트에 회귀 사례로 포함되어 있습니다. 실제 브라우저의 HTML/텍스트 변환과 `$$` 보존도 검사하고, 오류 대체 없이 렌더링되는 `tmp/pdfs/regression/ipad-recovery.pdf`를 추가 생성합니다. 실제 iPad 클립보드 캡처 전체를 대신하는 테스트는 아닙니다. `붙여넣기 원본 저장`은 앱이 붙여넣기 이벤트에서 원본을 확보한 경우에만 나타납니다.
